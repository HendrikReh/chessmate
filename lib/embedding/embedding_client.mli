...
(** OpenAI embeddings client used by ingestion/worker paths to batch FEN requests
    with chunk guards and retry/backoff. *)
...
[@@ocaml.doc "Initialise a client targeting the given OpenAI endpoint with the supplied api_key."]
...
[@@ocaml.doc
  "Batch request embeddings for FEN strings using the OpenAI embeddings REST API. \
   Requests automatically retry on transient HTTP failures (429, 5xx, etc.) using \
   exponential backoff. Configure retry behaviour via the optional environment variables \
   OPENAI_RETRY_MAX_ATTEMPTS and OPENAI_RETRY_BASE_DELAY_MS. Requests are chunked \
   (defaults: 2048 inputs, ~120k characters) to respect provider limits; override via \
