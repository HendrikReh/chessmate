...
let truncate_question text =
  ...
  let limit = 160 in
  if String.length cleaned <= limit then cleaned else String.prefix cleaned limit ^ "..."
...
let cost_json usage =
  ...
  let reasoning_cost = cost_component usage.Usage.reasoning_tokens reasoning_rate in
  let total_cost =
    match List.filter_map [ input_cost; output_cost; reasoning_cost ] ~f:Fn.id with
    | [] -> None
    | values ->
        let sum = List.fold values ~init:0.0 ~f:( +. ) in
        if Float.(sum <= 0.) then None else Some sum
  in
  if Option.is_none total_cost && Option.is_none input_cost && Option.is_none output_cost && Option.is_none reasoning_cost then
    `Null
  else
    `Assoc
