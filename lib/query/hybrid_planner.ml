...
let match_clause key value =
  `Assoc
    [ "key", `String key
    ; "match", `Assoc [ "value", `String value ] ]

let range_clause key ~gte =
  `Assoc
    [ "key", `String key
    ; "range", `Assoc [ "gte", `Int gte ] ]
...
let convert_filter (filter : Query_intent.metadata_filter) =
  ...
  | "opening" -> Some (match_clause "opening_slug" (normalize_slug filter.value))
...
let build_payload_filters plan =
  let converted_filters =
    ...
    |> List.filter_map ~f:convert_filter
  in
...
let query_vector plan =
  ...
  List.init vector_dimension ~f:(fun index ->
      ...
      | _ ->
          let total =
            List.fold tokens ~init:0.0 ~f:(fun acc token -> acc +. hash_component token index)
          in
          clamp_float (total /. Float.of_int (List.length tokens)) ~min:0.0 ~max:1.0)
...
let vector_hit_of_point (point : Repo_qdrant.scored_point) =
  ...
  | Some payload -> (
      ...
      | Some game_id ->
          ...
          let keywords = Util.member "keywords" payload |> string_list_of_json in
...
let index_hits_by_game hits =
  List.fold hits ~init:(Map.empty (module Int)) ~f:(fun acc hit -> Map.set acc ~key:hit.game_id ~data:hit)
...
let merge_phases base additional =
  merge_keywords base additional

let merge_themes base additional =
  merge_keywords base additional
