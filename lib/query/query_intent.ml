...
let limit_from_tokens tokens =
  let qualifier_words =
    Set.of_list
      (module String)
      [ "top"; "first"; "show"; "list"; "give"; "find"; "return" ]
  in
  let rec loop tokens prev_token =
    ...
    | token :: rest -> (
        let next_token =
          match rest with next :: _ -> Some next | [] -> None
        in
        match int_of_token token with
        | Some value when value > 0 && value <= 50 ->
            let qualifies =
              Option.value_map prev_token ~default:false ~f:(fun prev ->
                  Set.mem qualifier_words prev)
              || Option.value_map next_token ~default:false ~f:(fun next ->
                     String.equal next "games" || String.equal next "game")
            in
...
let metadata_from_phrases cleaned =
  ...
  let manual_filters =
    ...
    |> List.filter_map ~f:(fun (variants, filter) ->
           if
             List.exists variants ~f:(fun phrase ->
                 String.is_substring cleaned ~substring:phrase)
           then Some filter
           else None)
  in
  let opening_filters =
...
let extract_keywords tokens =
  let rec loop tokens seen acc =
    ...
    | [] -> List.rev acc
    | token :: rest ->
        if Set.mem stopwords token || String.length token <= 2 then
          loop rest seen acc
...
let parse_rating tokens =
  ...
  let rec loop tokens state =
    ...
    | token :: rest -> (
        ...
        in
        let previous_tokens =
          update_previous state.State.previous_tokens token
        in
        match int_of_token token with
        | Some value ->
            let diff_context =
            ...
            in
            let min_context =
              List.exists state.State.previous_tokens ~f:(fun prev ->
                  Set.mem min_context_words prev)
            in
            let rating =
              if diff_context then state.State.rating
              else if min_context then
                update_rating state.State.rating color value
        ...
        | None ->
            let rating =
              ...
              | ("lower" | "less"), Some value ->
                  { state.State.rating with max_rating_delta = Some value }
              | _ -> state.State.rating
            in
            let pending_number =
              if relevant_context token then state.State.pending_number
              else None
            in
            loop rest
              State.
