  ...
  let rec loop i state =
    if i >= len then ()
      ...
      | `Normal ->
          ...
          | _ -> Stdlib.Buffer.add_char buffer char; loop (i + 1) `Normal)
      | `Brace -> if Char.(char = '}') then loop (i + 1) `Normal else loop (i + 1) `Brace
      | `Paren -> if Char.(char = ')') then loop (i + 1) `Normal else loop (i + 1) `Paren
      | `Line_comment -> if Char.(char = '\n') then (Stdlib.Buffer.add_char buffer char; loop (i + 1) `Normal) else loop (i + 1) `Line_comment
  in
  loop 0 `Normal;
...
let parse_header_line line =
  ...
  | Some rest -> (match String.chop_suffix rest ~suffix:"]" with
      | None -> Or_error.errorf "Invalid PGN header line: %s" line
      | Some inner ->
          ...
          | key :: value_parts ->
              let value_raw = String.concat ~sep:" " value_parts |> String.strip in
              let len = String.length value_raw in
              let value =
                if len >= 2 && Char.(value_raw.[0] = '"') && Char.(value_raw.[len - 1] = '"') then
                  String.sub value_raw ~pos:1 ~len:(len - 2)
                else
                  value_raw
              in
              Ok (key, value))
...
let rec collect_headers_and_moves lines headers =
  ...
  | line :: rest ->
      ...
      if String.is_empty trimmed then collect_headers_and_moves rest headers
        ...
        Or_error.bind (parse_header_line trimmed) ~f:(fun header -> collect_headers_and_moves rest (header :: headers))
      else
    ...
    let rec loop tokens acc current_turn ply =
      ...
      | token :: rest ->
          ...
          if String.is_empty token then loop rest acc current_turn ply
            ...
            let rec count_digits idx = if idx < len && Char.is_digit token.[idx] then count_digits (idx + 1) else idx in
            ...
            let new_turn, body =
              if digit_count > 0 then
                ...
                | Some turn_num ->
                    let remainder = String.sub token ~pos:digit_count ~len:(len - digit_count) in
                    let remainder = drop_while remainder ~f:(Char.equal '.') |> String.strip in
                    (turn_num, remainder)
              else
                (current_turn, token)
            in
            ...
            if String.is_empty body then
              let updated_turn = if Int.(new_turn > 0) then new_turn else current_turn in
              loop rest acc updated_turn ply
            else
              let next_ply = ply + 1 in
              let effective_turn = if Int.(new_turn > 0) then new_turn else ((next_ply + 1) / 2) in
              let move = { san = body; turn = effective_turn; ply = next_ply } in
              let next_turn = if Int.(next_ply % 2 = 0) then effective_turn + 1 else effective_turn in
...
let parse raw_pgn =
  ...
  Or_error.bind (collect_headers_and_moves lines []) ~f:(fun (headers, move_lines) ->
