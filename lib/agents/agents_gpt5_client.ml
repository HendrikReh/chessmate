...
module Verbosity = struct
  ...
  let to_string = function Low -> "low" | Medium -> "medium" | High -> "high"
...
module Response_format = struct
  ...
  let to_json = function
    ...
    | Json_schema schema ->
        `Assoc [ ("type", `String "json_schema"); ("json_schema", schema) ]
end
...
let create ~api_key ?(endpoint = default_endpoint) ?(model = default_model)
    ?(default_effort = Effort.Medium) ?default_verbosity () =
...
let create_from_env () =
  ...
  in
  let endpoint =
    Option.value (non_empty_env "AGENT_ENDPOINT") ~default:default_endpoint
  in
  let model =
    Option.value (non_empty_env "AGENT_MODEL") ~default:default_model
  in
  let* default_effort =
        ...
        | `List entries -> (
            let texts =
              ...
              |> List.filter_map ~f:(fun entry ->
                     ...
                     | `String "text" ->
                         Util.member "text" entry |> Util.to_string_option
                     | _ -> None)
            in
            match texts with
            | [] -> loop rest
...
let reasoning_field effort =
  `Assoc
    [ ("reasoning", `Assoc [ ("effort", `String (Effort.to_string effort)) ]) ]
...
let verbosity_field verbosity =
  ...
  | Some level ->
      [
        ( "response",
          `Assoc [ ("verbosity", `String (Verbosity.to_string level)) ] );
      ]
...
let build_payload ~model ~messages ~effort ~verbosity ~max_output_tokens
    ~response_format =
  let base_fields =
    [
      ("model", `String model);
...
let generate t ?reasoning_effort ?verbosity ?max_output_tokens ?response_format
  ...
  if List.is_empty messages then
    ...
    let body =
      build_payload ~model:t.model ~messages ~effort ~verbosity
    ...
    let attempt ~attempt:_ =
      ...
      | Ok { status; _ } when Common.should_retry_status status ->
          let err =
            Error.of_string (Printf.sprintf "GPT-5 transient status %d" status)
          in
          Backoff.Retry err
      | Ok { status; body } when status < 200 || status >= 300 ->
          let message =
            Printf.sprintf "GPT-5 request failed with status %d: %s" status
          ...
          | json -> (
              ...
              | `Null -> (
                  let content_result =
                    ...
                    | None ->
                        Or_error.error_string
                          "GPT-5 response did not contain any text output"
                  in
                  ...
                  | Ok content ->
                      ...
                      Backoff.Resolved
                        (Or_error.return
                           Response.{ content; usage; raw_json = json }))
              | error_json ->
                  let message =
                    Util.member "message" error_json
                    |> Util.to_string_option
                    |> Option.value ~default:"unknown error"
                  in
                  let err =
                    Error.of_string (Printf.sprintf "GPT-5 error: %s" message)
                  in
                  if Common.should_retry_error_json error_json then
                    Backoff.Retry err
    ...
    Backoff.with_backoff ~max_attempts:t.retry.max_attempts
      ...
      ~on_retry:(fun ~attempt ~delay err ->
        Common.log_retry ~label:"openai-agent" ~attempt
          ~max_attempts:t.retry.max_attempts ~delay err)
      ~f:attempt ()
