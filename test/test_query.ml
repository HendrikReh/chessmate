...
let test_query_intent_opening () =
  ...
  check (option int) "rating delta" (Some 100)
    plan.Query_intent.rating.max_rating_delta;
  let filters = to_filter_pairs plan.Query_intent.filters in
  let has_opening_filter =
    List.exists filters ~f:(fun (field, value) ->
        String.equal field "opening"
        && String.equal value "kings_indian_defense")
  in
  check bool "opening filter" true has_opening_filter;
  check bool "keyword includes indian" true
    (List.mem plan.Query_intent.keywords "indian" ~equal:String.equal)
...
let test_query_intent_draw () =
  ...
  let expect_pairs =
    [
      ("opening", "french_defense"); ("phase", "endgame"); ("result", "1/2-1/2");
    ]
  in
  List.iter expect_pairs ~f:(fun expected ->
      let has_filter =
        List.mem filters expected ~equal:(fun (a1, b1) (a2, b2) ->
...
let test_hybrid_executor_merges_vector_hits () =
  ...
  | Ok execution ->
      ...
      check bool "phases merged" true
        (List.mem result.phases "middlegame" ~equal:String.equal);
      check bool "themes merged" true
        (List.mem result.themes "tactics" ~equal:String.equal);
      check bool "keywords merged" true
        (List.mem result.keywords "indian" ~equal:String.equal);
      check bool "vector score propagated" true
        Float.(abs (result.vector_score -. 0.92) < 1e-6)
...
let test_hybrid_executor_warns_on_vector_failure () =
  ...
  | Ok execution ->
      check bool "warnings emitted" true
        (not (List.is_empty execution.Hybrid_executor.warnings));
      let result = List.hd_exn execution.Hybrid_executor.results in
...
let test_hybrid_executor_with_agent () =
  ...
  | Ok execution ->
      ...
      | Some score ->
          check bool "agent score recorded" true
            Float.(abs (score -. 0.9) < 1e-6)
      | None -> fail "missing agent score");
      check bool "agent explanation present" true
        (Option.is_some first.agent_explanation);
      check bool "agent themes propagated" true
        (List.mem first.agent_themes "attack" ~equal:String.equal);
      check bool "final ranking" true
        Float.(first.total_score > second.total_score)
...
let test_hybrid_executor_agent_cache () =
  ...
  | Ok execution -> (
      ...
      | Some score ->
          check bool "agent score present" true
            Float.(abs (score -. 0.7) < 1e-6)
...
let test_agent_response_parse_valid () =
  let json =
    "{\"evaluations\": [{\"game_id\": 42, \"score\": 0.87, \"explanation\": \
     \"Sharp attack\", \"themes\": [\"attack\", \"sacrifice\"]}]}"
  in
  ...
  | Ok items ->
      ...
      check bool "score" true
        Float.(abs (item.Agent_response.score -. 0.87) < 1e-6);
      check (option string) "explanation" (Some "Sharp attack")
        item.Agent_response.explanation;
      check (list string) "themes" [ "attack"; "sacrifice" ]
        item.Agent_response.themes
...
let suite =
  [
    ...
    ( "hybrid executor warns on vector failure",
      `Quick,
    ...
    ( "agent response parser invalid",
      `Quick,
      test_agent_response_parse_invalid_evaluations_type );
  ]
