...
let test_fixture_fens_normalize () =
  ...
  | Error err -> failf "failed to derive FENs from fixture: %s" (Error.to_string_hum err)
  | Ok fens ->
      List.iteri fens ~f:(fun idx fen ->
          match Fen.normalize fen with
          | Ok _ -> ()
          | Error err ->
              failf "fixture FEN #%d failed normalization: %s" (idx + 1) (Error.to_string_hum err))

let suite =
  [ test_case "normalizes valid initial position" `Quick test_valid_initial_position
  ...
  ; test_case "rejects invalid rank description" `Quick test_invalid_rank_square_count
  ...
  ; test_case "rejects inconsistent en passant rank" `Quick test_invalid_en_passant_rank
  ; test_case "rejects duplicate castling rights" `Quick test_duplicate_castling_rights
