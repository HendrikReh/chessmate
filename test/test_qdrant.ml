...
let test_upsert_and_search () =
  ...
  let test () =
    ...
    | Ok () -> ()
    | Error err -> failf "expected upsert success but got %s" (Error.to_string_hum err));
    (match Repo_qdrant.vector_search ~vector:[ 0.01; 0.02; 0.03 ] ~filters:None ~limit:1 with
    | Ok [ hit ] ->
        check string "vector id" "vector-123" hit.Repo_qdrant.id;
        (match hit.payload with
        | Some payload ->
            let game_id = Yojson.Safe.Util.(payload |> member "game_id" |> to_int) in
    ...
    | Error err -> failf "expected search success but got %s" (Error.to_string_hum err))
  in
