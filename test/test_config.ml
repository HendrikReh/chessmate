...
let with_env overrides f =
  ...
  Exn.protect
    ~f
    ~finally:(fun () -> List.iter saved ~f:(fun (name, value) -> set_env name value))
...
let test_api_config_success () =
  with_env
    ...
    (fun () ->
      ...
      | Error err -> failf "unexpected config failure: %s" (Error.to_string_hum err)
      | Ok config ->
          check string "database" "postgres://user:pass@localhost:5432/db" config.Config.Api.database_url;
          check string "qdrant" "http://localhost:6333" config.Config.Api.qdrant_url;
          ...
          | other -> failf "expected low effort, got %s" (Agents_gpt5_client.Effort.to_string other));
...
let test_worker_config_missing_openai () =
  with_env
    ...
    (fun () ->
      ...
      | Error err ->
          check bool "mentions OPENAI_API_KEY"
            true
            (String.is_substring (Error.to_string_hum err) ~substring:"OPENAI_API_KEY"))

let test_cli_guard_limit () =
  with_env [ "CHESSMATE_MAX_PENDING_EMBEDDINGS", Some "500000" ] (fun () ->
      ...
      | Error err -> failf "unexpected guard failure: %s" (Error.to_string_hum err))

let suite =
  [ test_case "api config loads" `Quick test_api_config_success
  ; test_case "api config fails when database missing" `Quick test_api_config_missing_database
  ; test_case "worker config requires openai key" `Quick test_worker_config_missing_openai
